...
typedef TMilli BcastC$Forward_timer$precision_tag;
typedef TMilli BcastC$Timer$precision_tag;
...
static void BcastC$Boot$booted(void );
# 83 "/opt/tinyos-2.1.2/tos/lib/timer/Timer.nc"
static void BcastC$Forward_timer$fired(void );
# 110 "/opt/tinyos-2.1.2/tos/interfaces/AMSend.nc"
static void BcastC$AMSend$sendDone(
#line 103
message_t * msg, 
...
error_t error);
# 75 "/opt/tinyos-2.1.2/tos/interfaces/TaskBasic.nc"
static void BcastC$init$runTask(void );
# 78 "/opt/tinyos-2.1.2/tos/interfaces/Receive.nc"
static 
#line 74
message_t * 
...
static void BcastC$Control$startDone(error_t error);
#line 138
static void BcastC$Control$stopDone(error_t error);
# 83 "/opt/tinyos-2.1.2/tos/lib/timer/Timer.nc"
static void BcastC$Timer$fired(void );
...
# 80 "/opt/tinyos-2.1.2/tos/interfaces/AMSend.nc"
static error_t /*BcastAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$AMSend$send(am_addr_t addr, 
#line 71
message_t * msg, 
...
static void /*BcastAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$Send$sendDone(
#line 96
message_t * msg, 
...
# 73 "/opt/tinyos-2.1.2/tos/lib/timer/Timer.nc"
static void BcastC$Forward_timer$startOneShot(uint32_t dt);
# 80 "/opt/tinyos-2.1.2/tos/interfaces/AMSend.nc"
static error_t BcastC$AMSend$send(am_addr_t addr, 
#line 71
message_t * msg, 
...
uint8_t len);
# 67 "/opt/tinyos-2.1.2/tos/interfaces/TaskBasic.nc"
static error_t BcastC$init$postTask(void );
# 104 "/opt/tinyos-2.1.2/tos/interfaces/SplitControl.nc"
static error_t BcastC$Control$start(void );
# 64 "/opt/tinyos-2.1.2/tos/lib/timer/Timer.nc"
static void BcastC$Timer$startPeriodic(uint32_t dt);
#line 78
static void BcastC$Timer$stop(void );
# 32 "BcastC.nc"
enum BcastC$__nesc_unnamed4333 {
#line 32
  BcastC$init = 3U
};
#line 32
typedef int BcastC$__nesc_sillytask_init[BcastC$init];
#line 21
message_t BcastC$forward_packet[1000];
message_t BcastC$source_packet[1000];
bool BcastC$busy[1000];
uint16_t BcastC$node_id[1000];
uint16_t BcastC$seq_num[1000];
msg_t *BcastC$rec_msg[1000];
msg_t BcastC$cache[1000][MAX_ARRAY_SIZE];
uint16_t BcastC$index[1000];
int BcastC$counter[1000];


static inline void BcastC$init$runTask(void );
#line 44
static inline void BcastC$Boot$booted(void );
...
static inline void BcastC$Control$startDone(error_t error);
#line 68
static inline void BcastC$Control$stopDone(error_t error);

static inline void BcastC$Timer$fired(void );
#line 109
static inline void BcastC$Forward_timer$fired(void );
#line 141
static void BcastC$AMSend$sendDone(message_t *msg, error_t error);
#line 155
static inline message_t *BcastC$Receive$receive(message_t *msg, void *payload, uint8_t len);
# 53 "/opt/tinyos-2.1.2/tos/chips/atm128/timer/HplAtm128TimerCtrl8.nc"
...
# 110 "/opt/tinyos-2.1.2/tos/interfaces/AMSend.nc"
static void /*BcastAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$AMSend$sendDone(
#line 103
message_t * msg, 
...
error_t error);
# 75 "/opt/tinyos-2.1.2/tos/interfaces/Send.nc"
static error_t /*BcastAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$Send$send(
#line 67
message_t * msg, 
...
static void /*BcastAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$AMPacket$setDestination(
#line 99
message_t * amsg, 
...
am_addr_t addr);
#line 162
static void /*BcastAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$AMPacket$setType(
#line 158
message_t * amsg, 
...
static error_t /*BcastAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$AMSend$send(am_addr_t dest, 
message_t *msg, 
uint8_t len);
...
static inline void /*BcastAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$Send$sendDone(message_t *m, error_t err);
# 80 "/opt/tinyos-2.1.2/tos/interfaces/AMSend.nc"
...
# 73 "/opt/tinyos-2.1.2/tos/lib/timer/Timer.nc"
inline static void BcastC$Forward_timer$startOneShot(uint32_t dt){
#line 73
...
#line 158
  if (len == sizeof(msg_t )) 
    {
      BcastC$rec_msg[sim_node()] = (msg_t *)payload;
      sim_log_debug(163U, "Receive", "receive %d %d, time: %s\n", __nesc_ntoh_uint16(BcastC$rec_msg[sim_node()]->nodeid.nxdata), __nesc_ntoh_uint16(BcastC$rec_msg[sim_node()]->seq_num.nxdata), sim_time_string());
      if (__nesc_ntoh_uint16(BcastC$rec_msg[sim_node()]->nodeid.nxdata) == BcastC$node_id[sim_node()]) 
        {
          return msg;
        }
      for (i = 0; i < MAX_ARRAY_SIZE; i++) 
        {
          if (__nesc_ntoh_uint16(BcastC$cache[sim_node()][i].nodeid.nxdata) == __nesc_ntoh_uint16(BcastC$rec_msg[sim_node()]->nodeid.nxdata) && __nesc_ntoh_uint16(BcastC$cache[sim_node()][i].seq_num.nxdata) >= __nesc_ntoh_uint16(BcastC$rec_msg[sim_node()]->seq_num.nxdata)) 
            {
              return msg;
            }
        }

      BcastC$cache[sim_node()][BcastC$index[sim_node()]] = *BcastC$rec_msg[sim_node()];

      BcastC$index[sim_node()] = (BcastC$index[sim_node()] + 1) % MAX_ARRAY_SIZE;

      BcastC$Forward_timer$startOneShot(BcastC$node_id[sim_node()] % 10);
    }
...
#line 78
    case 20:
#line 78
      __nesc_result = BcastC$Receive$receive(msg, payload, len);
#line 78
      break;
#line 78
    default:
...
# 64 "/opt/tinyos-2.1.2/tos/lib/timer/Timer.nc"
inline static void BcastC$Timer$startPeriodic(uint32_t dt){
#line 64
...
#line 64
# 104 "/opt/tinyos-2.1.2/tos/interfaces/SplitControl.nc"
inline static error_t BcastC$Control$start(void ){
#line 104
  unsigned char __nesc_result;
#line 104
...
static inline void BcastC$Control$startDone(error_t error)
{
  ...
  if (error != SUCCESS) 
    {
      BcastC$Control$start();
    }
  else 
    {
      BcastC$node_id[sim_node()] = TOS_NODE_ID;
      i = rand() % 5000;
      i++;
      BcastC$Timer$startPeriodic(i);
    }
}
...
#line 113
  BcastC$Control$startDone(error);
#line 113
}
#line 113
# 96 "/opt/tinyos-2.1.2/tos/lib/tossim/TossimPacketModelC.nc"
...
static inline void BcastC$init$runTask(void )
{
  ...
  for (i = 0; i < MAX_ARRAY_SIZE; i++) 
    {
      __nesc_hton_uint16(BcastC$cache[sim_node()][i].nodeid.nxdata, 0);
      __nesc_hton_uint16(BcastC$cache[sim_node()][i].seq_num.nxdata, 0);
    }
}
...
# 110 "/opt/tinyos-2.1.2/tos/interfaces/AMSend.nc"
inline static void /*BcastAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$AMSend$sendDone(message_t * msg, error_t error){
#line 110
  BcastC$AMSend$sendDone(msg, error);
#line 110
}
#line 110
...
#line 65
{
  /*BcastAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$AMSend$sendDone(m, err);
}
...
# 100 "/opt/tinyos-2.1.2/tos/interfaces/Send.nc"
inline static void /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$Send$sendDone(uint8_t arg_0x40be06a8, message_t * msg, error_t error){
#line 100
  switch (arg_0x40be06a8) {
#line 100
    case 0U:
#line 100
      /*BcastAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$Send$sendDone(msg, error);
#line 100
      break;
#line 100
    default:
#line 100
      /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$Send$default$sendDone(arg_0x40be06a8, msg, error);
#line 100
      break;
#line 100
    }
...
#line 67
# 80 "/opt/tinyos-2.1.2/tos/interfaces/AMSend.nc"
inline static error_t /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$AMSend$send(am_id_t arg_0x40bde0a0, am_addr_t addr, message_t * msg, uint8_t len){
#line 80
  unsigned char __nesc_result;
#line 80

#line 80
  __nesc_result = TossimActiveMessageC$AMSend$send(arg_0x40bde0a0, addr, msg, len);
#line 80
...
# 78 "/opt/tinyos-2.1.2/tos/lib/timer/Timer.nc"
inline static void BcastC$Timer$stop(void ){
#line 78
...
#line 78
# 80 "/opt/tinyos-2.1.2/tos/interfaces/AMSend.nc"
inline static error_t BcastC$AMSend$send(am_addr_t addr, message_t * msg, uint8_t len){
#line 80
  unsigned char __nesc_result;
#line 80

#line 80
  __nesc_result = /*BcastAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$AMSend$send(addr, msg, len);
#line 80
...
# 126 "/opt/tinyos-2.1.2/tos/interfaces/Packet.nc"
inline static void * BcastC$Packet$getPayload(message_t * msg, uint8_t len){
#line 126
  void *__nesc_result;
#line 126
...
static inline void BcastC$Timer$fired(void )
{
  ...
  if (BcastC$busy[sim_node()] == TRUE) 
    {
      return;
    }

  if (BcastC$node_id[sim_node()] % 2 == 1 && BcastC$counter[sim_node()] < 5) 
    {
      msg_t *bcast_msg;
      int i;

      bcast_msg = (msg_t *)BcastC$Packet$getPayload(&BcastC$source_packet[sim_node()], sizeof(msg_t ));
      BcastC$counter[sim_node()]++;

      if (bcast_msg == (void *)0) 
        {
          return;
        }
      BcastC$seq_num[sim_node()]++;
      __nesc_hton_uint16(bcast_msg->nodeid.nxdata, BcastC$node_id[sim_node()]);
      __nesc_hton_uint16(bcast_msg->seq_num.nxdata, BcastC$seq_num[sim_node()]);

      if (BcastC$AMSend$send(AM_BROADCAST_ADDR, &BcastC$source_packet[sim_node()], sizeof(msg_t )) == SUCCESS) 
        {

          BcastC$busy[sim_node()] = TRUE;
        }

      sim_log_debug(158U, "Send", "send %d %d, time: %s\n", __nesc_ntoh_uint16(bcast_msg->nodeid.nxdata), __nesc_ntoh_uint16(bcast_msg->seq_num.nxdata), sim_time_string());
    }
  else {
#line 102
    if (BcastC$counter[sim_node()] == 5) 
      {
        BcastC$Timer$stop();
        sim_log_debug(159U, "Send", "Timer stopped\n");
      }
    }
}
...
static inline void BcastC$Forward_timer$fired(void )
{
  ...
  bcast_msg = (msg_t *)BcastC$Packet$getPayload(&BcastC$forward_packet[sim_node()], sizeof(msg_t ));
  ...
  __nesc_hton_uint16(bcast_msg->nodeid.nxdata, __nesc_ntoh_uint16(BcastC$rec_msg[sim_node()]->nodeid.nxdata));
  __nesc_hton_uint16(bcast_msg->seq_num.nxdata, __nesc_ntoh_uint16(BcastC$rec_msg[sim_node()]->seq_num.nxdata));


  if (BcastC$AMSend$send(AM_BROADCAST_ADDR, &BcastC$forward_packet[sim_node()], sizeof(msg_t )) == SUCCESS) 
    {
      BcastC$busy[sim_node()] = TRUE;
    }

  sim_log_debug(160U, "Send", "send %d %d\n", __nesc_ntoh_uint16(bcast_msg->nodeid.nxdata), __nesc_ntoh_uint16(bcast_msg->seq_num.nxdata));
}
...
# 83 "/opt/tinyos-2.1.2/tos/lib/timer/Timer.nc"
inline static void /*HilTimerMilliC.VirtualizeTimerC*/VirtualizeTimerC$0$Timer$fired(uint8_t arg_0x40b723a8){
#line 83
  switch (arg_0x40b723a8) {
#line 83
    case 0U:
#line 83
      BcastC$Timer$fired();
...
#line 83
    case 1U:
#line 83
      BcastC$Forward_timer$fired();
#line 83
      break;
#line 83
    default:
#line 83
      /*HilTimerMilliC.VirtualizeTimerC*/VirtualizeTimerC$0$Timer$default$fired(arg_0x40b723a8);
#line 83
      break;
#line 83
    }
...
# 103 "/opt/tinyos-2.1.2/tos/interfaces/AMPacket.nc"
inline static void /*BcastAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$AMPacket$setDestination(message_t * amsg, am_addr_t addr){
#line 103
...
inline static void /*BcastAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$AMPacket$setType(message_t * amsg, am_id_t t){
#line 162
...
# 75 "/opt/tinyos-2.1.2/tos/interfaces/Send.nc"
inline static error_t /*BcastAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$Send$send(message_t * msg, uint8_t len){
#line 75
  unsigned char __nesc_result;
#line 75
...
#line 62
# 67 "/opt/tinyos-2.1.2/tos/interfaces/TaskBasic.nc"
inline static error_t BcastC$init$postTask(void ){
#line 67
  unsigned char __nesc_result;
#line 67

#line 67
  __nesc_result = SimSchedulerBasicP$TaskBasic$postTask(BcastC$init);
#line 67
...
static void BcastC$AMSend$sendDone(message_t *msg, error_t error)
{
  if (&BcastC$source_packet[sim_node()] == msg) 
    {
      sim_log_debug(161U, "Send", "BUSY\n");
      BcastC$busy[sim_node()] = FALSE;
    }
  else {
#line 148
    if (&BcastC$forward_packet[sim_node()] == msg) 
      {
        sim_log_debug(162U, "Send", "BUSY\n");
        BcastC$busy[sim_node()] = FALSE;
      }
    }
}
...
#line 55
{
  /*BcastAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$AMPacket$setDestination(msg, dest);
  /*BcastAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$AMPacket$setType(msg, 20);
  return /*BcastAppC.AMSenderC.SenderC.AMQueueEntryP*/AMQueueEntryP$0$Send$send(msg, len);
}
                               ...
                               uintptr_t* addr, size_t* size)
{
  ...
  /* Module BcastC */
  if (!strcmp(varname, "BcastC$forward_packet"))
  {
    *addr = (uintptr_t)&BcastC$forward_packet[__nesc_mote];
    *size = sizeof(BcastC$forward_packet[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "BcastC$source_packet"))
  {
    *addr = (uintptr_t)&BcastC$source_packet[__nesc_mote];
    *size = sizeof(BcastC$source_packet[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "BcastC$busy"))
  {
    *addr = (uintptr_t)&BcastC$busy[__nesc_mote];
    *size = sizeof(BcastC$busy[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "BcastC$node_id"))
  {
    *addr = (uintptr_t)&BcastC$node_id[__nesc_mote];
    *size = sizeof(BcastC$node_id[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "BcastC$seq_num"))
  {
    *addr = (uintptr_t)&BcastC$seq_num[__nesc_mote];
    *size = sizeof(BcastC$seq_num[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "BcastC$rec_msg"))
  {
    *addr = (uintptr_t)&BcastC$rec_msg[__nesc_mote];
    *size = sizeof(BcastC$rec_msg[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "BcastC$cache"))
  {
    *addr = (uintptr_t)&BcastC$cache[__nesc_mote];
    *size = sizeof(BcastC$cache[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "BcastC$index"))
  {
    *addr = (uintptr_t)&BcastC$index[__nesc_mote];
    *size = sizeof(BcastC$index[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "BcastC$counter"))
  {
    *addr = (uintptr_t)&BcastC$counter[__nesc_mote];
    *size = sizeof(BcastC$counter[__nesc_mote]);
    return 0;
  }
...
static void __nesc_nido_initialise(int __nesc_mote)
{
  ...
  /* Module BcastC */
  memset((void *)&BcastC$forward_packet[__nesc_mote], 0, sizeof BcastC$forward_packet[__nesc_mote]);
  memset((void *)&BcastC$source_packet[__nesc_mote], 0, sizeof BcastC$source_packet[__nesc_mote]);
  BcastC$busy[__nesc_mote] = FALSE;
  BcastC$node_id[__nesc_mote] = 0;
  BcastC$seq_num[__nesc_mote] = 0;
  memset((void *)&BcastC$rec_msg[__nesc_mote], 0, sizeof BcastC$rec_msg[__nesc_mote]);
  memset((void *)&BcastC$cache[__nesc_mote], 0, sizeof BcastC$cache[__nesc_mote]);
  BcastC$index[__nesc_mote] = 0;
  BcastC$counter[__nesc_mote] = 0;
